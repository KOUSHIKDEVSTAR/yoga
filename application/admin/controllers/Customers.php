...

{
    ...
    public function view($id){
        ...
        $data['getCustomerPlanById']=$this->model_customers->getCustomerPlanById($id);

        $this->data['customer_plan']=$data['getCustomerPlanById'];
    ...
    public function take_payment(){
        ...
        $this->db->where('sch_cust_id',$cust_id);
        $amount_rest  = $this->db->get()->result();
        $rest_amount = intval($amount_rest[0]->amount) - intval($amount);
        $data =[
            'amount'=>$rest_amount,
        ];
        $this->db->where('sch_cust_id',$cust_id);
        $res = $this->db->update('tbl_cancel_boking_sch',$data);

        $data = [

            'pay_desc'=>"Wallet Balance",
            ...
            'payment_note'=>'Payment For Penlaty',

        ];
        $this->db->insert('tbl_wallet_transction',$data);
        if($res){
            return redirect('customers/view/'.$cust_id.'');
        }
